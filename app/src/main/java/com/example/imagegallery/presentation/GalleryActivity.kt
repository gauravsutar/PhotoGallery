...
class GalleryActivity : AppCompatActivity() {
    ...
    private fun initViews() {
        ...
        binding.recyclerviewGallery.addItemDecoration(GridSpacingItemDecoration(spacingInPixels))
        setupScrollListener()
    ...
    private fun updateListFromInput() {
        binding.edtSearch.text.trim().let {
            if (it.isNotEmpty()) {
                ...
                adapter.clearList()
                viewModel.searchPhotos(binding.edtSearch.text.toString(), 1)
            }
        }
    }
    ...
    private fun showErrorInformation(count: Int, message: String? = getString(R.string.default_no_results_msg)) {
        message?.let {
